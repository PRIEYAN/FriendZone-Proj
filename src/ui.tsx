...
export function ui(): ReactEcs.JSX.Element {
  ...
  return (
    ...
    >
      {header(constellation.name, constellation.blurb, drawn, total, solved)}
...
/** Constellation name, a pip per line, the blurb, and the clock. */
function header(
  name: string,
  blurb: string,
  drawn: number,
  ...
  return (
    ...
    >
      <Label value={name} fontSize={S.title} color={INK} />
      {pips(drawn, total)}
      {!solved ? <Label value={blurb} fontSize={S.small} color={DIM} /> : null}
      <Label
        value={
          best > 0
...
/**
 * The payoff. Names the constellation, then the mythic figure that just bloomed
 * behind it, then the time — and calls out a new dome record, which is the
 * cheapest reason to run the board again.
 */
function banner(name: string, figure: string): ReactEcs.JSX.Element {
  ...
  return (
    <UiEntity
      ...
      uiBackground={{ color: Color4.create(0.06, 0.05, 0.02, 0.85) }}
    >
      <Label value={`${name} — Complete`} fontSize={S.banner} color={GOLD} />
      {figure !== '' ? <Label value={figure} fontSize={S.body} color={CYAN} /> : null}
      <Label
        value={
          record
            ? `${formatTime(lastRunSeconds())} — new dome record`
            : formatTime(lastRunSeconds())
        }
        fontSize={S.body}
        color={record ? GOLD : DIM}
      />
      <Button
        value='Next sky'
        fontSize={S.body}
 ...
 * reserved for the client's own action button on mobile.
 */
function footer(solved: boolean): ReactEcs.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <UiEntity
        uiTransform={{ padding: { top: 8, bottom: 8, left: 14, right: 14 } }}
        uiBackground={{ color: PANEL }}
      >
